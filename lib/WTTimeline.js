var WTTimeline = function(childTasks, $timeline) {
...
WTTimeline.prototype.noSubtasksMsg = function() {
	var l = '52px';//!this.ctx.parent ? '36px': '52px';
	$msg = $('<div></div>');//$('<div class="msg"><i class="fa fa-exclamation"></i> Task has no subtasks</div>');	
...
WTTimeline.prototype.sortTasks = function(tasks) {
	//var me = this;
	/*var tasks = [];
...
WTTimeline.prototype.generateSelectedExplorer = function(taskIds) {
	if(!taskIds) return [];
	var me = this;
	var taskTree = [];
	$.each(me.sortTasks(taskIds), function(k, taskId) {			
...
WTTimeline.prototype.initData = function( ) {
	...
	//me.ctx = me.generateSelectedExplorer();
	me.subtasks = [];
	var tasks = me.childTasks;
	
	$.each(tasks, function(k,t) {
		var task = {};
		task.id = t.id;
		task.title = t.name;
		task.meta = (!t.metaData.startDate || !t.metaData.endDate);
		task.warning = t.metaData.isInconsistent;
		task.progress = t.metaData.progress/100;
		if (t.metaData.endDate) {
			task.overdue = (t.metaData.endDate < new Date() && t.metaData.progress < 99.9);
		}
		if (!task.meta) {
			task.start = t.metaData.startDate/1000;
			task.target = t.metaData.endDate/1000;
			task.type = "medium";
		}
...
WTTimeline.prototype.drawMetaBar = function(subtask, posy, posyc, barHeight) {
	...
	var metabarwidth = 30;	  
   	...
   	}else if(subtask.progress>0.99){
   		var meta = me.paper.path("M "+xstart+" "+posy+" l "+metabarwidth+" 0 l 0 "+barHeight+" l -"+metabarwidth+" 0 z");
	   	meta.attr({'stroke': me.doneColor(subtask), 'stroke-width': 1});
  	}else{
...
WTTimeline.prototype.textColor = function(task) {
	if(task.warning)
		...
		if(task.overdue)
			...
			return this.color.GREEN_DARKDARK;
};
